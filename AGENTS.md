...
```bash
# 验证所有 skill（自动发现 + 基线检查 + 各 skill 的 validate.sh）
python3 check.py

# 仅基线检查，跳过各 skill 的 validate.sh
python3 check.py --smoke

# 微信排版：Markdown → 微信公众号 HTML
...
python3 scripts/select_style.py --title "主题" --usage "封面"
...
| Skill | 类型 | 职责 |
|-------|------|------|
| `kairos-wechat-typeset` | 渲染型 | Markdown → 微信公众号可粘贴 HTML，5 套主题（song/wending/tech/wisme/pi） |
| `kairos-visual-generator` | 渲染型 | 主题 → 视觉卡片/封面 PNG，4 套视觉系统（editorial/swiss/mondrian/ticket） |
| `kairos-loop` | 对话型 | 帮用户写出规范、可运行的 loop prompt（自循环任务） |
| `kairos-serenity` | 对话型 | 基于 @aleabitoreddit 的 AI 供应链瓶颈框架做投资推荐/评估/诊断 |
| `kairos-x-scraper` | 对话型 | 抓取 X.com 公开推文，按博主持久化，不做任何提炼 |
