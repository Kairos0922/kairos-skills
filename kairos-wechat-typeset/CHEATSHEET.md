...
1. 标题是第一视觉焦点吗？（不是 → 检查 H1 字号是否够大）
2. 颜色克制吗？（超过 3 种功能色 → 检查主题 JSON）
3. 有呼吸感吗？（连续 3 段以上纯文字 → 加 Divider 或 Quote）
4. 像人工精修的，还是像 AI 拼凑的？（后者 → 检查 anti-patterns）
5. 手机上看起来舒服吗？（在 390px 宽度下检查）
