...
| 档位 | 说明 | 示例 |
| --- | --- | --- |
| **端到端** | 文本输入 → HTML 输出，全流程自动 | Markdown 文章 → 排版好的 HTML，含主题选择、语义分析、渲染和验证 |
| **文强图弱** | 文本处理全自动，配图依赖宿主 agent | 需要配图的文章（agent 需有生图能力，skill 只验证 image-plan.json） |
| **能力圈外** | 不适合用此 skill | 动态交互页面、PPT/PDF 导出、封面图生成、自定义 CSS/颜色、运行时新增主题 |

### 不做什么

| 不做 | 原因 |
| --- | --- |
| 不替代微信平台标题、封面图、账号信息 | 只管正文排版 |
| 不允许用户传自定义 CSS / 颜色 / 主题文件 | 保护美学一致性 |
