...
:::lead
留白不是空，克制不是少，而是在适当的空间中，让呼吸发生，让眼睛愿意继续往下读。
:::
...
3. 第三条有序列表，后面跟一段续行这一句是列表项的续行文本，用来测试缩进和换行后的对齐。
...
![一张用于观察图片说明与边距的占位图](https://placehold.co/1200x720/f1e8dc/3f3932.png?text=Kairos+Typeset)

图片需要保持块级、移动端安全，并让说明文字像杂志图注一样克制。

:::figure
![水墨山间的单列正文示意图](https://placehold.co/1200x900/e9e1d5/3f3932.png?text=Song+Editorial)
一张图只承担一个编辑意图：把读者带入文章的气息，而不是制造装饰噪音。
:::

## 06 代码块

下面是一段 Python 代码，用来观察代码标签、代码底色、行高和中英文混排。

```python
def render_theme(theme_id: str, markdown: str) -> str:
    theme = load_theme(theme_id)
    return render_markdown_text(markdown, theme=theme)
```
