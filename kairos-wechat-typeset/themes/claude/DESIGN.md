...
Claude 主题用于教程、解释型文章、文档、方法论、产品说明和技术观点。它以附件中的 Claude Design Language 母版为准：Paper 背景、Ink / Stone / Paper 三色系统、Instrument Serif 式大标题、Inter 式正文、8px spacing system、细线分区和可扫描内容组件。

Claude 主题不是聊天界面截图，也不是通用文档皮肤。它要尽可能贴近参考图中可落地到微信公众号正文的部分：大号衬线标题、微暖纸面、浅 Stone 组件、细线结构、低饱和图片、引用符号、代码块、任务清单、表格和底部提示条。

## Visual Language

- 关键词：Paper、Ink、Stone、Instrument Serif、Inter、fine rules、calm intelligent reading、clarity and focus。
...
- Font: serif-first for brand/title/headings; Inter-like sans for body, metadata, lists, and tables; mono for code.
- Type scale from reference: H1 52px / 1.1, H2 28px / 1.25, H3 20px, body 16px / 1.75, caption/code/list 13px / 1.6.
- Spacing: base unit 8px; common steps 8, 16, 24, 40, 64, 96.

## Components

- H1: 52px serif editorial title, left aligned, close to the sample article headline.
- H2 numeric: small `01` label plus 28px serif heading, separated by fine rules when needed.
- H3-H6: 20px serif subheads.
- Paragraph: Inter-like sans body, 16px / 1.75.
...
- Code: Paper/Stone code panel with 13px mono text, thin border and small language label.
- Inline code: subtle Stone chip at 13px.
- Table: true fine-line grid in inline spans, matching the reference table more closely than stacked cards.
- Lists: small Inter markers, vertically aligned. Task lists render checked/unchecked square boxes.
...
- Audit should keep the primary type scale bounded to 13px, 16px, 20px, 28px, and 52px.
